...
contract CyclerFarm is IRewardDistributionRecipient, ReentrancyGuard {
    ...
    modifier isNotContract {
        ...
        // solhint-disable-next-line no-inline-assembly
        assembly { codehash := extcodehash(account) }
        require((codehash != accountHash && codehash != 0x0) == false, "Contracts are not allowed to interact with the farm");
        _;
    }
    ...
    function recoverLostTokensExceptOurTokens(IERC20 _token, uint256 amount) external ownerOnly {
    ...
    function farm(uint256 amount) external isNotContract updateReward(msg.sender) nonReentrant {
    ...
    function withdraw(uint256 amount) public isNotContract updateReward(msg.sender) nonReentrant {
    ...
    function getReward() public isNotContract updateReward(msg.sender) {
